...
interface EmptySlotProps {
	index: number
	isOpen: boolean
	onOpenChange: (open: boolean, slot: number) => void
		...
		>
			<CardContent className="flex flex-col items-center justify-center h-48 p-6">
				<Dialog
					open={isOpen}
					onOpenChange={(open) => {
						onOpenChange(open, index)
						if (!open) onSearchChange("")
					}}
				>
					<DialogTrigger asChild>
						<Button variant="outline" className="h-24 w-24 rounded-full border-dashed border-2">
						...
						<div className="space-y-3">
							...
							<div className="flex flex-row gap-2 items-start sm:items-center justify-between">
								...
								<div className="flex flex-row gap-1">
									...
									>
										{sortType === "alphabetical" && reverseSort && (
							...
							<div className="flex flex-col sm:flex-row gap-4 items-center">
								...
								>
									{heroClasses.map((heroClass) => (
										<label
											key={heroClass.value}
											htmlFor={`class-${heroClass.value}`}
											className="flex items-center space-x-1 md:space-x-2 cursor-pointer"
										>
											<RadioGroupItem value={heroClass.value} id={`class-${heroClass.value}`} />
											{heroClass.value !== "all" ? (
												<Image
													src={heroClass.icon}
													alt={heroClass.name}
