...
export function createWeaponVisibilitySync(
	modelFiles: ModelFile[],
	loadedModels: ReadonlyMap<string, Object3D>,
) {
	const swordFile = modelFiles.find((file) =>
	...
	const sheathFile = modelFiles.find((file) =>
		file.path.startsWith(`${hero}/`) && (file.type === "weaponb" || file.type === "weapon_b"),
	)
	...
	return (visibleModels: ReadonlySet<string>, attachedWeapons: ReadonlySet<string>) => {
		...
		const embeddedSwordShown = visibleModels.has(sheathFile.name) && sheath.visible &&
			Math.max(Math.abs(embeddedSword.scale.x), Math.abs(embeddedSword.scale.y), Math.abs(embeddedSword.scale.z)) > 0.01
